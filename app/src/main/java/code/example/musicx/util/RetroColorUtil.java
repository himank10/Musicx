...
public class RetroColorUtil {
  ...
  public static int getTextColor(@Nullable Palette palette) {
    ...
    if (inverse != -1) {
      return code.example.appthemehelper.util.ColorUtil.INSTANCE.getReadableText(inverse, background, 150);
    }
    return code.example.appthemehelper.util.ColorUtil.INSTANCE.stripAlpha(getSwatch(palette).getTitleTextColor());
  }
  ...
  public static int shiftBackgroundColorForLightText(@ColorInt int backgroundColor) {
    while (code.example.appthemehelper.util.ColorUtil.INSTANCE.isColorLight(backgroundColor)) {
      backgroundColor = code.example.appthemehelper.util.ColorUtil.INSTANCE.darkenColor(backgroundColor);
    }
